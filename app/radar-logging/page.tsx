...
import type { NormalizedPostData, RadarPerson, RadarTraceStep } from "@/app/api/radar/route"
import {
  ChartContainer,
  ChartTooltip,
  ChartTooltipContent,
...
export default function PostRadarLoggingPage() {
  ...
  const [trace, setTrace] = React.useState<RadarTraceStep[] | null>(null)
  ...
  const handleSearch = React.useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/radar/logging", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: postUrl.trim() }),
      })
      const data = await res.json().catch(() => ({}))
      const steps = (data as { trace?: RadarTraceStep[] }).trace ?? []
      setTrace(steps)
      if (!res.ok) {
        setError((data as { error?: string }).error || res.statusText || "Search failed")
        return
      }
      const result = data as RadarResult & { trace?: RadarTraceStep[] }
  ...
  return (
    <AppShell title="Post Radar Logging">
      <div className="px-4 lg:px-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Same flow as Post Radar, plus a step-by-step log of APIs called (Supabase / Unipile), request payloads,
              and response summaries. Secrets are not included.
            </p>
          </div>
        ...
        {(trace && trace.length > 0) || loading ? (
          <Card>
            <CardHeader>
              <CardTitle>API trace</CardTitle>
              <CardDescription>
                Order of operations for this search. Unipile uses three parallel GETs per URN attempt: post, comments,
                reactions.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 max-h-[min(70vh,720px)] overflow-y-auto">
              {loading && (!trace || trace.length === 0) ? (
                ...
                (trace ?? []).map((step) => (
                  <div
                    key={step.order}
                    className="rounded-md border bg-muted/20 p-3 text-xs font-mono space-y-1.5 break-all"
                  >
                    <div className="flex flex-wrap items-center gap-2 text-foreground">
                      <span className="font-semibold tabular-nums">#{step.order}</span>
                    ...
                    {step.httpMethod && (
                      <div>
                        <span className="text-muted-foreground">HTTP:</span> {step.httpMethod}
                      </div>
                    )}
                    <div>
                      <span className="text-muted-foreground">Endpoint:</span> {step.endpoint}
                    </div>
                    <div>
                    ...
                    {(step.responseStatus != null || step.responseSummary) && (
                      <div>
                        <span className="text-muted-foreground">Response:</span>{" "}
                        {step.responseStatus != null ? `status ${step.responseStatus}` : ""}
                        {step.responseSummary ? ` — ${step.responseSummary}` : ""}
                      </div>
                    )}
                  </div>
                ))
